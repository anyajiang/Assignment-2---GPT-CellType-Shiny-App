...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("dataset", "Select Seurat Differential Gene Table", choices = list("pbmc_small" = "1", "list" = "2", "others" = "3"), selected = "pbmc_small"),
      selectizeInput("model", label = "Select Model", 
                     choices=c(c("gpt-3.5-turbo", "gpt-4")), selected="gpt-4", multiple=F),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$annotate, {
    ...
    if (input$dataset == 1) {
      ...
      res <- gptcelltype(input = list(gs1=c('CD4,CD3D'),gs2='CD14'), tissuename=NULL, model='gpt-4', topgenenumber = 10) 
